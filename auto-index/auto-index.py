...
def generate_index(root, dnames, fnames, depth):
    ...
    if args.external_style:
        style = f"""<link href=\"/static/index-style.css\" rel=\"stylesheet\">
        <link href=\"/static/favicon.ico\" rel=\"stylesheet\">"""
        # depth is irrelevant when serving static files with jinja2 with a static
        ...
        if entry.name not in IGNORES and os.path.splitext(entry.name)[1] not in IGNORE_EXTS:
            ...
            if entry.stat().st_mode & stat.S_IROTH:
                if entry.is_dir():
                    ...
                    fnames.append(f"{{{{ path }}}}/{entry.name}")
                else:
